...
These settings are specified in the file[path]_mobicents-ussdgateway-/jboss-5.1.0.GA/bin/run.conf_. 
...
You will also find a lot of examples in the folder [path]_mobicents-ussdgateway-/jboss-5.1.0.GA/docs/examples/jca/_. 

[[_configure_jslee_jdbc_ra]]
=== Configuring JSLEE JDBC RA

{this-platform} {this-application} leverages JSLEE JDBC RA for persistence.
Detailed JSLEE JDBC RA documentation is available in [path]_mobicents-ussdgateway-/docs/slee/Mobicents_SLEE_RA_JDBC_User_Guide.pdf_ that explains how to point to the new DataSource.
...
You must change the Dialect in the file [path]_mobicents-ussdgateway-/jboss-5.1.0.GA/server/<profile>/deploy/mobicents-slee/META-INF/jboss-beans.xml_ to reflect your alternate DataSource. 
...
Please refer to the SLEE RA HTTP Client User Guide available in [path]_mobicents-ussdgateway-/docs/slee/Mobicents_SLEE_RA_HTTP_Client_User_Guide.pdf_. 
...
Logging is controlled from a central configuration file located at [path]_mobicents-ussdgateway-/jboss-5.1.0.GA/server/<profile>/conf/jboss-log4j.xml_, one for each JBoss AS configuration profile.
...
Open a Web Browser, navigate to http://localhost:8080/mobicents-management/ and switch to the 'USSD GW' tab.
...
. The main panel will display the existing Server Settings (if any), segregated into three tabs: Error Messages, SS7 Settings, Various.
  Switch to the 'SS7 Settings' tab in the GUI. 
. In the text field 'USSD Gateway subsystem number', you can set a value for USSD Sub System Number (SSN). Issuing this command in CLI will set the SSN value but you must ensure that the SSN value is properly configured in the TCAP Stack in the xml descriptor file [path]_mobicents-ussdgateway-version/jboss-5.1.0.GA/server/<profile>/deploy/mobicents-ussd-gateway/META-INF/jboss-beans.xml_.
  For more details of this parameter, please refer to the description of the CLI command for the same in the preceding section. 
. You must click on the button 'Apply Changes' to save your settings.
  If there is an error in setting the value, then you will find the details of the error in the Management Console Log section below. 
