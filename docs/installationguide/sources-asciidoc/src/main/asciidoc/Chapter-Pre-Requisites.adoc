...
Development Kit (JDK) installed on your system and it must be version 5 
or higher. If you intend to use SIGTRAN (M3UA) then you must have JDK 7 
installed. M3UA uses Java SCTP which is available only from JDK 7 
onwards. JAVA 8 is not supported now. | If you are using M3UA, then you must use JDK 7 to run the stack as well as to compile the source code. For TDM cards JDK 7 is also recommended.
